...
export function postConfirmIdentification(req: Request, res: Response) {
  const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const edits = req.body && Object.keys(req.body).length > 0 ? req.body : undefined;
  const item = confirmIdentification(id, edits);
  if (!item) return res.status(404).json({ error: "Item not found" });
  return res.status(200).json(item);
}
...
export function getComparables(req: Request, res: Response) {
  const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const comparables = getItemComparables(id);
